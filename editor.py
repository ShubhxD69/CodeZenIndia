...
class CodeEditor(QPlainTextEdit):
    ...
    def keyPressEvent(self, event: QKeyEvent):       # noqa: N802
        ...
        # Shift, Ctrl, Alt, Meta alone must never trigger autocomplete or
        # interfere with normal editor behaviour.
        _MODIFIER_KEYS = (
        ...
        # Pass navigation keys to the completer popup ONLY when no Shift held.
        # Shift+Enter must perform smart-indent, not accept a completion.
        if self._completer.popup().isVisible():
            ...
            if event.key() in (
                ...
                event.ignore()
                return
            # Shift+Enter while popup visible → hide popup and do smart return
            if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter) and shift_held:
                self._completer.popup().hide()
                self._smart_return()
                return
